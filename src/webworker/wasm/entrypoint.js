...
function JsOnAwaitWGpuWork(handle) { const data = Emval.toValue(handle); const { type, width, height, ptrPixels, srcWidth, srcHeight, srcPtrPixels } = data; const pixels = new Uint8Array(Module.HEAPU8.buffer, ptrPixels, width * height * 4); const taskData = { type, width, height, pixels }; if (typeof srcWidth === 'number' && typeof srcHeight === 'number' && typeof srcPtrPixels === 'number') { taskData.srcWidth = srcWidth; taskData.srcHeight = srcHeight; taskData.srcPixels = new Uint8Array(Module.HEAPU8.buffer, srcPtrPixels, srcWidth * srcHeight * 4); } console.log("[WebGpuDemoFn] Received task:", taskData, Module.wgpuTaskPool); const workId = Module.wgpuTaskPool.registerTask(taskData); const retValue = { workId, }; return Emval.toHandle(retValue); }
