...
async function CreateUjWasmModule(moduleArg = {}) {
  var moduleRtn;
...
function findWasmBinary() {
  ...
  return new URL('entrypoint.wasm', import.meta.url).href;
}
...
function JsOnCaptureBitmap(slotIdStr,modeStr,imageData) { const jSlotIdStr = UTF8ToString(slotIdStr); const jModeStr = UTF8ToString(modeStr); const jImageData = Emval.toValue(imageData); const clone = new ImageData( new Uint8ClampedArray(jImageData.data), jImageData.width, jImageData.height ); globalThis.pipelineEvents.dispatchEvent(new CustomEvent("BITMAP_CAPTURED", { detail: { assetKey: jSlotIdStr, mode: jModeStr, imageData: clone } })); }
JsOnCaptureBitmap.sig = 'viii';
function JsOnDestroyBitmap(pixelData) { Module._free(pixelData); }
JsOnDestroyBitmap.sig = 'vi';
function JsOnAwaitWGpuWork(handle) { const data = Emval.toValue(handle); const { type, targetImage: rawTargetImage, inImages: rawInImages = [], fragShader } = data; const toRuntimeImage = (rawImage) => { if (!rawImage || typeof rawImage.width !== 'number' || typeof rawImage.height !== 'number') { return null; } let pixels = rawImage.pixels; if (typeof pixels === 'number') { pixels = new Uint8Array(Module.HEAPU8.buffer, pixels, rawImage.width * rawImage.height * 4); } return { width: rawImage.width, height: rawImage.height, pixels, }; }; const targetImage = toRuntimeImage(rawTargetImage); const inImages = Array.isArray(rawInImages) ? rawInImages.map(toRuntimeImage).filter((img) => img !== null) : []; const taskData = { type, targetImage, inImages, fragShader }; const taskId = Module.awaitPool.addTask("wgpu", taskData); console.log("[WebGpuDemoFn] Received task:", taskId, taskData); return Emval.toHandle({ taskId }); }
JsOnAwaitWGpuWork.sig = 'ii';
...
export default CreateUjWasmModule;
