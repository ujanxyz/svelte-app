...
import type { WebGpuResources } from './WebGpuResources';
import {
...
export async function RunWgpuPipeline(
  task: WgpuTask,
  fragShader: string,
  webGpuResources: WebGpuResources,
): Promise<void> {
  ...
  } = await webGpuResources.getResources();
  ...
  const fragmentModule = device.createShaderModule({ code: fragShader });
  ...
  pass.setVertexBuffer(0, fullscreenQuadVertexStripBuffer);
  if (bindGroup) {
